...
import { ChangePasswordDto, ForgotPasswordDto, LoginDto, RefreshTokenDto, RegisterCompleteDto, RegisterInitiateDto, RegisterVerifyOtpDto, ResetPasswordDto } from './dto/auth.dto';
...
export class AuthController {
     ...
     * 
     * @param postData - Containing mobile number, otp and new password
     * @returns The response object cotains the success or failure message.
     */
    @Post("reset-password")
    async resetPassword(@Body() postData: ResetPasswordDto){
        try {
            return this.authService.resetPassword(postData);
        } catch (error: any) {
            return ErrorException(
                HttpStatus.INTERNAL_SERVER_ERROR,
                error.message,
